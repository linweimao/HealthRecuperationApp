...
import com.lwm.healthrecuperationapp.adapter.MyCollectAdapter;
...
public class MyCollectActivity extends BaseActivity implements OnItemChildClickListener {
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    if (datas.size() > 0) {
                        mEmptyLinear.setVisibility(View.GONE);
                    } else {
                        // 显示缺省页面
                        mEmptyLinear.setVisibility(View.VISIBLE);
                    }
                    // 将数据设置进 MyCollectAdapter
                    mMyCollectAdapter.setDatas(datas);
                    mMyCollectAdapter.notifyDataSetChanged(); // 通知 RecyclerView 刷新页面(刷新数据)
                    break;
            }
        }
    };
    ...
    protected void initData() {
        ...
        mMyCollectAdapter.setOnItemChildClickListener(this);
